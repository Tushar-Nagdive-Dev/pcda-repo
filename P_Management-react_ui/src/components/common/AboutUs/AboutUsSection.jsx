...
import organizationChart from '@/assets/images/OrganisationChart.jpg'
     ...
     {aboutusData.map((item, index) => (
      <React.Fragment key={item.id}>
       {index === 3 ? (
        <DialogTrigger>
         <div
          className={`flex  text-titleColor  items-center gap-14 p-12 cursor-pointer transition-all  ${
           currentStage !== index
          ...
          <h3
           className={`${
            currentStage !== index
             ? `text-titleColor`
             : `text-statebluecolor`
           } text-2xl font-bold max-w-[350px]`}
          >
           {item.title}
          </h3>
         </div>
         ...
         {aboutusData.map((item, index) => (
          <React.Fragment key={item.id}>
           {currentStage === index && (
            <React.Fragment>
             {currentStage === 3 ? (
              <DialogContent className="max-w-[1250px] h-fit p-0" onCloseAutoFocus={() => setCurrentStage(0)}>
               <div className="w-auto flex items-center justify-center">
                <img
                 src={item.image}
                 alt={item.title}
                 className={`h-full w-full ${currentStage === 3 ? 'object-fill' : 'object-cover'} rounded-lg`}
                />
               </div>
              </DialogContent>
             ) : (
              ...
              >
               <div className="h-full flex gap-16 overflow-y-auto">
                <div className="w-full flex flex-col space-y-8 text-ternaryGrey font-medium ">
                 ...
                 <p className="text-justify max-h-[250px] ">
                  {item.description}
                 </p>
                </div>
               </div>
