...
function RTIIDASTable({ transitionStyles, state, currentStage }) {
 return (
  ...
  >
   <h3 className="text-3xl font-semibold text-titleColor mb-10 text-center">रक्षा लेखा प्रधान नियंत्रक(अफसर) कार्यालय के स्कन्ध अधिकारी एवं उनके फोन नंबर</h3>
   <Table>
    <TableHeader>
     <TableRow>
      <TableHead className="w-[100px] font-bold text-xl">Sr.No.</TableHead>
      <TableHead className="font-bold text-xl">Name of the Officer</TableHead>
      <TableHead className="w-[350px] font-bold text-xl">Phone Number</TableHead>
     </TableRow>
    </TableHeader>
    <TableBody>
     {RTIIDASData.map((row, index) => (
      <React.Fragment key={row.sr_no}>
       <TableRow>
        ...
        <TableCell className={`text-xl`}>{row.name_of_officer}</TableCell>
